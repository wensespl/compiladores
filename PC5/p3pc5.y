...
%token ASIGNAR SUMAR RESTAR MULTIPLICAR DIVIDIR PARENTESIS BUCLE IMPRIMIR
%token SI SINO ENTONCES FINSI SALTARF1 SALTARF2 MIENTRAS HACER FINMIENTRAS
...
instr: SI cond {
      ...
      | MIENTRAS {bucleW = cx;} '(' cond ')' {generaCodigo(SALTARF1, $4,'?','-');
                                              $$=cx;} HACER bloque {generaCodigo(BUCLE, bucleW, '-', '-');} {TCodigo[$5].a3=cx;} FINMIENTRAS;
instr: IMPRIMIR expr { printf("%lf", $2); };
bloque: instr listaInstr;
